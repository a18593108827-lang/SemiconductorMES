...
- **MP 的 `eq(col, null)` 不会跳过条件**（源码证据 `AbstractWrapper:467-470`：`addCondition` 只判 condition 布尔、**不判 val 是否为 null**）→ 生成 `col = NULL`，SQL 语义恒为 UNKNOWN → **判重/查询静默失效**。可空字段参与条件必须写 `.eq(val != null, col, val)`，null 场景用 `.isNull(col)`。现网判重先例 = `selectCount(…)` + `AssertUtil.isTrue(count == 0, "……已存在")`（`AuthServiceImpl:43-45`）；UK 冲突兜底另有捕捉 `DuplicateKeyException` 沿异常链取约束名的写法（`ComplaintPackageFacadeImpl:560-569`）
...
- **CI 约定（2026-09-24，EVAL-0002 回流）**：① 被 CI 调用的脚本**禁止硬编码绝对路径**（按 `__file__` 推断仓库根）② 被 `git diff --exit-code` 比对的生成物**禁止含 `date.today()`** 等时钟输入 —— 「同一天跑得通」不等于幂等（隔天必假失败）③ 改动 CI 步骤后**必须在真实 runner 上验证一次**，本地通过不作为依据。背景：docs 闸门自落地起就因硬编码路径 + 非幂等日期而一直失败（`docs/eval/EVAL-0002-CI文档闸门失效（硬编码路径+生成物非幂等）.md`）
