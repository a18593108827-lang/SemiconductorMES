...
> **现状（2026-08-12）**：EDC **设计已定 / 实现未建**；SPC 未建。  
...
| 业务 | 优先级 | 说明 |
|------|--------|------|
| 量测数据录入 | P1 | 站点 EDC（**模块待建**；门禁前置） |
| 规格上下限校验 | P1 | Spec / Control Limit（随 EDC） |
| 与 TrackOut 卡控 | P1 | 不合格不可过站（T2-7 ⏸；依赖 EDC，**不依赖 SPC**） |
...
| 业务 | 优先级 | 说明 |
|------|--------|------|
| Track 事务履历 | P0 | 谁/何时/何站/何机 |
| 正向追溯 | P0 | Lot → 下游 |
| 反向追溯 | P0 | 成品 → 原料/设备/Recipe |
| 设备加工履历 | P0 | Lot 在机历史 |
| Recipe / 参数履历 | P1 | 加工时实际配方 |
