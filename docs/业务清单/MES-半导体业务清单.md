...
> **厂型已定（2026-09-23）：聚焦后道封测**（含功率 / SiC 车规）；前道列远期 —— 依据 `docs/方案/MES-厂型选型分析.md`。§16 优先级已上调主线，§17 降为远期  
...
## 16. 封测特化（OSAT，按需）— **P0/P1（2026-09-23 上调：当前主线）**

> 厂型**已定（2026-09-23）：聚焦后道封测**（依据 `docs/方案/MES-厂型选型分析.md`）。本表三项核心已由 P1 上调至 **P0**。

| 业务 | 优先级 | 说明 |
|------|--------|------|
| Die / Strip / Tray 层级 | **P0** | 封装单元；颗级追溯的基础 |
| 测试程序 / Bin 结果 | **P0** | 电测分 Bin 回流并与批次绑定 |
| 不良 Bin 映射 | **P0** | 与 Hold/Rework 联动（质量拦截） |
| 打标 / 外观检对接 | P2 | 后道自动化 |
| 编带 / 包装入库 | P2 | 出货衔接 |
| 客户 Lot 映射 | P1 | 来料与出货 Lot（追溯链闭环） |

---

## 17. 晶圆厂特化（Fab）— **远期（2026-09-23 降级：厂型未选前道）**

> 厂型定于后道封测（`docs/方案/MES-厂型选型分析.md`）；本表留档，待产线侧成熟后再评估（现实的入口是 8 寸 / 特色工艺，12 寸量产线不列入路线图）。
