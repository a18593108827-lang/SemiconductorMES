...
Carrier ✅ **C0+C1 与 C2 扫码比对已落地**（Car-1～8；`docs/模块/Carrier（载具）模块/`）；**Adapter（SECS/GEM）/ Alarm→Hold 策略 / 片级 Wafer 等本轮不动**。

### 下一步（2026-09-23 定：厂型 = 后道封测）

厂型决策落定：**聚焦后道封测**（含功率器件 / SiC 车规场景），依据 `docs/方案/MES-厂型选型分析.md`（§0 决策记录）。
...
| 项 | 选项 | 影响 | 当前建议 |
|----|------|------|----------|
| 厂型 | 前道 / 后道 / 封测 | 是否要片级 | ✅ **已定（2026-09-23）：后道封测**（含功率 / SiC 车规）；依据 `docs/方案/MES-厂型选型分析.md` |
