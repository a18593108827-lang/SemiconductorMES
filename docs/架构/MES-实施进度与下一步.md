...
| History | ✅ 一期 P0 + 客诉包 CP-4 | 写在 Track；`HistoryFacade` + `/app/history`；客诉包 build/get/list/export ✅；contain 后置 |
...
9. ~~**History（调查台）**~~ ✅ Facade + `/app/history` 真数据 + 设备反查；客诉包 CP-4 export ✅；contain 后置  
...
| History（履历） | MVP 常见 | 写 + 调查台 + 设备反查已落地 | 相符；客诉包 export ✅；片级 / contain 后置 |
