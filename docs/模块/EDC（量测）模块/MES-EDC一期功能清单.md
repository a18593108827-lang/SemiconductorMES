...
现状：采得进、判得了；**卡得住（TrackOut 门禁）未接**。FAIL 单也能完工。
...
| P0 | `EdcFacade` + evaluateGate | ⏳ |
| P0 | Admin 最小页 / API | ✅ `/app/edc` 特性/规格/站计划 |
| P0 | T2-7 TrackOut 钩子 + 现场拒出 | ⏳（录入已接；拒出依赖 Facade） |
...
下一步：**EDC-4 Facade**，再 EDC-6/7 拒出。
**禁止**跳过 EDC-4 直接在 Track 写点表。
...
| 键 | 默认 | 说明 |
|----|------|------|
| `mes.edc.gate-enabled` | true | false 时 Facade 对 required 站也 clear（应急）。**尚未落地** |
...
已可验：主数据、手录、OOS、同 visit 最新条查询、现场录入。
未可验：required 站拒 Out、FAIL 拒 Out。
