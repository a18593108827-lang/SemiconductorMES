...
| P1 | n≥25 才返回 Cpk；规格限仅展示 | 随 P0 序列接口带上 |
...
已有库上线前执行 `server/src/main/resources/db/migrate_spc.sql`（含 EDC 索引 + 权限种子）。
...
- 发布在采集事务提交后；`com.mes.edc` 零 `import com.mes.spc`  
- 本切片 **无** 判异、无新表、无前端  
...
见 `MES-SPC架构设计.md` §10。落地后勾 `MES-SPC已完成功能.md`。
