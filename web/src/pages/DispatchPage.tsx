          ...
          <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-border bg-surface">
            ...
            {candidates?.held ? (
              <div className="border-b border-danger/20 bg-danger/5 px-4 py-2.5 text-sm text-danger">
                {candidates.message || "批次已锁批"}
              </div>
            ) : null}
            {candidates?.message && !candidates.held && items.length === 0 ? (
              <div className="border-b border-border px-4 py-2.5 text-sm text-muted">{candidates.message}</div>
            ) : null}
